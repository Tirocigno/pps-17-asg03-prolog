...
import it.unibo.pps17.core.prolog.{GOL_THEORY_PATH, GameEngine, Goals, Variables}
...
class PrologToJavaConverterTest extends FunSuite with BeforeAndAfterEach {
  ...
  test("Conversion of a list") {
    val prologList = executeGoal(Goals.START_GAME_GOAL).getTerm(Variables.ALIVE_CELL_LIST)
  ...
  test("Conversion of a generation") {
    ...
    val generationIndex = solution.getTerm(Variables.GENERATION)
